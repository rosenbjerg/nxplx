	...
	public render(_, { libraries, createLibraryModalOpen }) {
		return (
			<S.Container>
							...
							<li key={lib.id} class="bordered" style="display: inline-block; padding: 2px 8px; margin: 2px">
								<div style="display: inline-grid; grid-template-columns: 32px auto;">
									...
									<div>
										...
										<button type="button" title={translate('index library')} onClick={this.indexLibrary(lib)} class="material-icons noborder"
												style="font-size: 18px;">refresh
										</button>
										<button type="button" title={translate('delete library')} onClick={this.deleteLibrary(lib)} class="material-icons noborder"
												style="font-size: 18px;">close
										</button>
									</div>
								</div>
							</li>
				...
				<S.BottomControls>
					<SecondaryButton onClick={this.indexAllLibraries}>{translate('index all libraries')}</SecondaryButton>
					<PrimaryButton onClick={() => this.setState({ createLibraryModalOpen: true })}>{translate('create library')}</PrimaryButton>
				</S.BottomControls>
			</S.Container>
		);
	}
