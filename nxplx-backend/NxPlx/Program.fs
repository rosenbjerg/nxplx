...
let indexSeries = Array.groupBy (fun (e:EpisodeEntry) -> e.name) >> Array.Parallel.map (fun seriesGroup ->
        ...
        let seasons = seriesEpisodes |> Array.groupBy (fun e -> e.season) |> Array.Parallel.map (fun seasonGroup ->
            ...
            let episodes = seasonEpisodes |> Array.map (fun e ->
                ...
                {id=tmdbEpisodeInfo.id; eid=e.id; number=e.episode; description=tmdbEpisodeInfo.overview; thumbnail=tmdbEpisodeInfo.still_path; aired=tmdbEpisodeInfo.air_date})
...
let indexFilm = Array.Parallel.map (fun f ->
        ...
        {id=info.id; eid=f.id; title=info.title; description=info.overview; released=info.release_date; poster=info.poster_path; background=info.backdrop_path}        
    )
...
let main argv =    
    ...
    server.Get("/api/film", (fun (req:Request) (res:Response) -> res.SendJson film))
    ...
    server.Get("/api/series", (fun (req:Request) (res:Response) -> res.SendJson series))
