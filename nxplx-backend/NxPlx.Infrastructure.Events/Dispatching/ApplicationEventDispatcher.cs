...
namespace NxPlx.Infrastructure.Events.Dispatching
{
    ...
    public class CachingApplicationEventDispatcher
    {
        ...
        public CachingApplicationEventDispatcher(IEventDispatcher eventDispatcher, IOperationContext operationContext, IDistributedCache distributedCache)
        {
            _eventDispatcher = eventDispatcher;
...
#if DEBUG
#else
            var cached = await _distributedCache.GetObjectAsync<TResult>(cacheKey, _operationContext.OperationCancelled);
